...
A hung InstantTab keeps its hotkeys registered with native Cmd+Tab off, which leaves the Mac without Cmd+Tab. So the host pings each tool every 2 seconds, and the tool answers on its main thread. One that misses 3 pings in a row, which is 6 to 8 seconds without an answer, or is not ready 12 seconds after it starts, is killed, and its exit is handled like a crash. InstantTab's Accessibility calls time out after half a second, so a tool that is only busy answers well within that. The checks are counted rather than timed, because the host's timer stops while the Mac sleeps and checks missed while the host itself is busy run once, and after a wake the tool gets 10 more seconds, since waking can stall it for a second or two.
