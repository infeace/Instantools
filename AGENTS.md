...
A macOS menu bar app of small tools where latency is the product, each in its own process: Cmd+Tab, an app switcher, and Language, a keyboard layout switcher on Control+Command. It is the successor to InstantTab and InstantLang and will replace them once it is ready. Pressing a tool's keys does no IPC and never waits on anything. Architecture and measurements are in docs/research.md.
...
- `Instantools` is the host: menu bar, Settings, Start at login, the tool supervisor and the first-launch migration.
- `InstantoolsCore` holds tool ids, host and tool messages, restart backoff and migration decisions. `InstantoolsKit` is shared by the host and the tools: the tool side of the channel, logging and permissions.
- `AppSwitcher` is the Cmd+Tab tool, `AppSwitcherCore` its pure logic, and `AppSwitcherKit` what Settings shares with it: the config store, displays, native Cmd+Tab and the panel style.
- `LayoutSwitcher` is the Language tool and `LayoutSwitcherCore` its pure logic.
- A new tool is a case in `ToolId`, its status in `ToolMessage`, an executable target, a line in the `tools` list of `scripts/build.sh`, and a Settings pane.
...
- Native Cmd+Tab must come back on every exit path of the Cmd+Tab tool: quit, pause, signals and crashes. The host restores it too after any exit it did not ask for.
- Only one copy of each tool may run: the host is single instance and stops leftover tools and the old apps before starting its own.
- Accessibility calls into a process's own windows must run on its main thread.
- Language's event tap is listen-only and never holds back input. TIS calls run on the main thread, and the tap sits on the main run loop so the switch happens inside its callback.
